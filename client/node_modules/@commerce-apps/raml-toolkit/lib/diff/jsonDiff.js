 ...
 */
exports.JSON_LD_KEY_GRAPH = "@graph";
exports.JSON_LD_KEY_CONTEXT = "@context";
exports.JSON_LD_KEY_ID = "@id";
exports.JSON_LD_KEY_TYPE = "@type";
/**
...
var DiffType;
(function (DiffType) {
    DiffType[DiffType["ADDED"] = 0] = "ADDED";
    DiffType[DiffType["REMOVED"] = 1] = "REMOVED";
    DiffType[DiffType["MODIFIED"] = 2] = "MODIFIED";
    DiffType[DiffType["TEXT_DIFF"] = 3] = "TEXT_DIFF";
    DiffType[DiffType["MOVED"] = 4] = "MOVED";
 ...
 */
function findJsonDiffs(left, right) {
    ...
    const jsonDiff = new jsondiffpatch_1.DiffPatcher({
    ...
    logger_1.ramlToolLogger.debug(`Added plugin to include node ID in the diff: ${jsonDiff.processor.pipes.diff.list()}`);
    /**
...
exports.findJsonDiffs = findJsonDiffs;
/**
...
function validateJson(obj) {
    ...
    const graph = obj[exports.JSON_LD_KEY_GRAPH];
    if (graph == null) {
        throw new Error(`${exports.JSON_LD_KEY_GRAPH} property is missing`);
    }
    if (!lodash_1.default.isArray(graph) || graph.length == 0) {
        throw new Error(`${exports.JSON_LD_KEY_GRAPH} property must be an array of json nodes`);
    }
}
/**
...
function parseDiffs(diffs) {
    const typedDiffs = [];
    if (lodash_1.default.isEmpty(diffs)) {
        //no changes
        logger_1.ramlToolLogger.info("No differences found by the jsondiffpatch");
        return typedDiffs;
    }
    logger_1.ramlToolLogger.debug("Parsing differences generated by jsondiffpatch");
    const graphDiffs = diffs[exports.JSON_LD_KEY_GRAPH];
    if (graphDiffs != null) {
        //process graph changes
        logger_1.ramlToolLogger.debug("Parsing differences in JSON-LD graph node");
        typedDiffs.push(...parseGraph(graphDiffs));
    }
    const contextDiffs = diffs[exports.JSON_LD_KEY_CONTEXT];
    if (contextDiffs != null) {
        //process context changes
        logger_1.ramlToolLogger.debug("Parsing differences in the JSON-LD context node");
        const typedContextDiff = parseNodePropDiffs(exports.JSON_LD_KEY_CONTEXT, contextDiffs);
 ...
 * @returns Array of NodeDiff objects
 */
function parseGraph(graphDiffs) {
    const typedDiffs = [];
    ...
    Object.keys(graphDiffs).forEach(key => {
        ...
        let typedDiff;
        if (lodash_1.default.isArray(diff)) {
        ...
        else if (lodash_1.default.isObject(diff)) {
            //properties in a node are added/removed/modified/moved
            typedDiff = parseNodePropDiffs(diff[exports.JSON_LD_KEY_ID], diff);
        }
        if (typedDiff != null) {
 ...
 */
function parseNodePropDiffs(nodeId, diff) {
    let typedDiff = new NodeDiff(nodeId, diff[exports.JSON_LD_KEY_TYPE]);
    Object.keys(diff)
        .filter(key => key !== exports.JSON_LD_KEY_ID && key !== exports.JSON_LD_KEY_TYPE) //ignore node id and type
        .forEach(key => {
        ...
        if (value[JSON_DIFF_PATCH_ARRAY_KEY] === JSON_DIFF_PATCH_ARRAY_VALUE) {
            ...
            Object.keys(value)
                ...
                logger_1.ramlToolLogger.debug(`Adding difference of node: ${typedDiff.id}, array property: ${key}, array key: ${arrKey}, difference type: ${DiffType[diffType]}`);
            ...
            logger_1.ramlToolLogger.debug(`Adding difference of node: ${typedDiff.id}, property: ${key}, difference type: ${DiffType[diffType]}`);
             ...
             */
            const diffType = getDiffType(value[exports.JSON_LD_KEY_ID]);
            logger_1.ramlToolLogger.debug(`Adding difference of node: ${typedDiff.id}, reference property: ${key}, difference type: ${DiffType[diffType]}`);
            addNodeReferenceDiffs(key, value[exports.JSON_LD_KEY_ID], diffType, typedDiff);
        }
    });
    if (lodash_1.default.isEmpty(typedDiff.added) && lodash_1.default.isEmpty(typedDiff.removed)) {
...
function getDiffType(diff) {
    ...
    throw new Error(`Invalid difference structure found: ${JSON.stringify(diff, null, 2)}`);
}
 ...
 */
function addNodeDiff(diff) {
    let typedDiff;
    const diffNode = diff[0];
    const id = diffNode[exports.JSON_LD_KEY_ID];
    const type = diffNode[exports.JSON_LD_KEY_TYPE];
    const diffType = getDiffType(diff);
    logger_1.ramlToolLogger.debug(`Parsing differences of node: ${id}, difference type: ${DiffType[diffType]}`);
    switch (diffType) {
        case DiffType.ADDED:
        ...
        case DiffType.REMOVED:
            ...
            break;
        case DiffType.MOVED:
            logger_1.ramlToolLogger.debug(`Ignoring the move of node: ${id}`);
            break;
        default:
            /**
             *  Note: MODIFIED, TEXT_DIFF are not valid for node as diff is by node id
             */
            throw new Error(`Invalid difference type for node: ${DiffType[diffType]}, node: ${id}`);
    }
    return typedDiff;
}
exports.addNodeDiff = addNodeDiff;
/**
 ...
 */
function addNodePropertyDiffs(key, diff, diffType, typedDiff) {
    switch (diffType) {
        case DiffType.ADDED:
        ...
        case DiffType.TEXT_DIFF:
            ...
            break;
        default:
            throw new Error(`Invalid difference type for node property: ${DiffType[diffType]},  node: ${typedDiff.id}, property: ${key}`);
    }
}
exports.addNodePropertyDiffs = addNodePropertyDiffs;
/**
 ...
 * @param typedDiff - Instance of NodeDiff for the node
 */
function addNodeArrayPropertyDiffs(key, diff, diffType, typedDiff) {
    switch (diffType) {
        case DiffType.ADDED:
        ...
        case DiffType.REMOVED:
            ...
            break;
        case DiffType.MOVED:
            logger_1.ramlToolLogger.debug(`Ignoring the moves of node: ${typedDiff.id}, property: ${key}`);
            break;
        default:
            throw new Error(`Invalid difference type for node array property value: ${DiffType[diffType]},  node: ${typedDiff.id}, property: ${key}`);
    }
}
exports.addNodeArrayPropertyDiffs = addNodeArrayPropertyDiffs;
/**
 ...
 * @param typedDiff - Instance of NodeDiff for the node
 */
function addNodeReferenceDiffs(key, diff, diffType, typedDiff) {
    switch (diffType) {
        case DiffType.MODIFIED:
            typedDiff.removed[key] = { [exports.JSON_LD_KEY_ID]: diff[0] };
            typedDiff.added[key] = { [exports.JSON_LD_KEY_ID]: diff[1] };
            break;
        case DiffType.TEXT_DIFF:
            //TODO: Handle text diffs where the text length exceeds the configured length
            break;
        default:
            throw new Error(`Invalid difference type for node reference property: ${DiffType[diffType]},  node: ${typedDiff.id}, property: ${key}`);
    }
}
/**
 * jsondiffpatch plugin to add node ID and type to the difference
 */
const addNodeInfo = function (context) {
    if (context.leftType === "object" &&
        typeof context.left[exports.JSON_LD_KEY_ID] !== "undefined") {
        const changed = !!lodash_1.default.find(context.children, childContext => childContext.result);
        if (changed) {
            context.setResult({
                /**
                 ...
                 */
                [exports.JSON_LD_KEY_ID]: context.right[exports.JSON_LD_KEY_ID],
                [exports.JSON_LD_KEY_TYPE]: context.right[exports.JSON_LD_KEY_TYPE]
            });
        }
    }
};
